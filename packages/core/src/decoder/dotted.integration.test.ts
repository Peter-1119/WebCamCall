...
describe('dotted Data Matrix (DPM)', () => {
  ...
  it('cycler walks dark then light kernels, then sticks to a success', () => {
    ...
    expect(c.next(640)).toEqual({ kernel: 13, polarity: 'light' })
    expect(c.next(640)).toEqual({ kernel: 17, polarity: 'light' })
    expect(seen.map((v) => `${v.polarity}${v.kernel}`)).toEqual(['dark5', 'dark9', 'dark13', 'dark17', 'light5', 'light9'])
  ...
  it('rgba/gray conversions round-trip and kernels scale with width', () => {
    ...
    expect(dottedKernels(640)).toEqual([5, 9, 13, 17])
    expect(dottedKernels(1280)).toEqual([11, 19, 27, 35])
    expect(dottedKernels(320)).toEqual([3, 5, 7, 9])
    expect(dottedKernels(520)).toEqual([5, 7, 11, 15])
  })
})
