...
`dotted` 開啟時，第一次正常解碼沒結果就做第二次：百分位對比拉伸 → 形態學膨脹（把點撐成方塊）→ 再解。膨脹核要接近模組大小，而模組大小取決於距離，所以每幀輪替 `[5, 9, 13, 17] × {暗點, 亮點}` 八種變體（依解碼影像寬度縮放），成功後記住。

- 預設 `'auto'`：`formats` 含 `data_matrix` 才開。`true`/`false` 強制。
- 只有 wasm 後端支援。
- 每幀成本 ~6–10 ms（van Herk 演算法，與核大小無關）。
- 實測 PCB 金面鑽孔碼：關閉 0/3 解出，開啟 3/3。
...
- 找不到回傳 `[]`，**不會 reject**。
- 沒有 ROI / debounce / 節流；`tryHarder` 一律開；`dotted` 會把所有變體試完。
- 座標以該影像原始尺寸為準。
