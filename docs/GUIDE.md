...
| `dotted` | 每幀 +6–10 ms（只在第一次沒解出時） | 只有 PCB 點陣碼需要 |
...
PCB 鑽孔、雷射點刻的 DPM 碼每個模組是**圓點**而非實心方格，zxing 取樣落在點間空隙 → 「有黃框但解不出」。

`dotted` 開啟時，第一次正常解碼沒結果就做第二次：百分位對比拉伸 → 形態學膨脹（把點撐成方塊）→ 再解。膨脹核要接近模組大小，而模組大小取決於距離，所以每幀輪替 `[5, 3, 9, 7, 13, 17] × {暗點, 亮點}` 十二種變體（**絕對值**，不隨畫面縮放——17 張 PCB 實拍證明核該跟模組走），成功後記住。

**即時掃描 vs 拍照模式的成本分工**（一般 QR / 一維流程完全不受影響）：

| | 每次多做的事 | 成本 |
|---|---|---|
| 即時掃描（`formats` 不含 `data_matrix`） | 無 | 0 |
| 即時掃描（含 `data_matrix`） | 第一次沒解出時：1 次膨脹 + 1 次解碼，每幀只試一個變體 | +6–20 ms／幀 |
| 拍照模式 `createDecoder` | 多尺度（960/1280/640/1920）× 12 變體 + 候選裁切，全部在 Worker 內同一份像素上跑 | 印刷碼 < 100 ms；點陣碼 0.2–1.5 s；預設 3 秒預算 |

- 預設 `'auto'`：`formats` 含 `data_matrix` 才開。`true`/`false` 強制。
- 只有 wasm 後端支援。
- 每幀成本 ~6–10 ms（van Herk 演算法，與核大小無關）。
- 實測 17 張 PCB 照片（兩種板材、遠近、旋轉、反光）：拍照模式 **11/17**（0.1.3 為 7/17）；剩下 6 張是整塊板子入鏡、碼只佔畫面 2% 的遠拍，模組不到 1 px，物理上不可解。

使用建議：碼佔框寬 **1/4 以上**、對準框中央、等約半秒讓它輪到正確的核。`createDecoder()`（單張圖片）沒有幀預算，會把八種變體全部試完。
